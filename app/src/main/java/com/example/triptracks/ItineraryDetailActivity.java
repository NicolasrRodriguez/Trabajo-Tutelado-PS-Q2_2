...
public class ItineraryDetailActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        itinerary = getIntent().getParcelableExtra(MainActivity.KEY_ITINERARY);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Establece los niveles de zoom basados en la especificidad
        float zoomLevel = 1.0f; // Zoom predeterminado para el mundo
        if (!TextUtils.isEmpty(countryName)) {
            zoomLevel = 6.0f; // Nivel de zoom para el país
        }
        if (!TextUtils.isEmpty(stateName)) {
            zoomLevel = 8.0f; // Nivel de zoom para el estado
        }
        if (!TextUtils.isEmpty(cityName)) {
            zoomLevel = 10.0f; // Nivel de zoom para la ciudad
        }
    ...
    private void configurar(ItineraryTileBinding binding) {
        ...
        for (Country country : MainActivity.mCountries) {
        ...
        binding.butBorrar.setOnClickListener(v -> {
            ...
            resultIntent.putExtra(MainActivity.KEY_ITINERARY, itinerary);
        ...
        binding.butOk.setOnClickListener(v -> {
            ...
            if (spinnerCountry.getSelectedItem() != null) {
                editedCountry = spinnerCountry.getSelectedItem().toString();
            }

            if (spinnerState.getSelectedItem() != null) {
                editedState = spinnerState.getSelectedItem().toString();
            }

            if (spinnerCity.getSelectedItem() != null) {
                editedCity = spinnerCity.getSelectedItem().toString();
            }
            editedCity = editedCity.equals(getString(R.string.select_city)) ? "" : editedCity;
            editedCountry = editedCountry.equals(getString(R.string.select_country)) ? "" : editedCountry;
            editedState = editedState.equals(getString(R.string.select_state)) ? "" : editedState;
            ...
            binding.itineraryCity.setText(itinerary.getCity());
            onMapReady(mMap);
            MainActivity.mAdapter.actualizar_por_id(itinerary);
